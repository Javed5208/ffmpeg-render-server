...
[v0]drawtext=text='Pyari Priya • Shop the look':x=20:y=25:fontcolor=white:fontsize=36:box=1:boxcolor=black@0.35:boxborderw=8
" -map "[v0]" -map 0:a -af "loudnorm=I=-14:LRA=7:TP=-2" -c:v libx264 -preset slow -b:v 3000k -c:a aac -b:a 128k -movflags +faststart ${output}
`
};
...
app.post("/render-final", async (req, res) => {
  try {
    ...
    const tmpl = templates[template] || templates['celebrity_aggressive'];
    const cmd = tmpl(inputTmp, outputTmp);
    await run(cmd);
    const buffer = fs.readFileSync(outputTmp);
    const uploaded = await uploadToSupabase(output_filename, buffer);
    try { fs.unlinkSync(inputTmp); fs.unlinkSync(outputTmp); } catch(e){}
